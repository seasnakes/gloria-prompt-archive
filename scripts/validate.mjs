...
assert.ok(!/(file_token|tenant_access_token|user_access_token|app_secret|record_id|https?:\/\/[^\s"<>]*\.(?:feishu\.cn|larksuite\.com))/i.test(text), 'Private source metadata in public catalog');
...
assert.ok(files.every(name => references.has('media/' + name)), 'Unreferenced media: run sync before publishing');
console.log(`Validated ${data.count} records, ${references.size} media files, ${(total / 1024 / 1024).toFixed(1)} MB. No private attachment credentials.`);
